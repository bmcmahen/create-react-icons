...
async function cli() {
  program
    ...
    .option(
      "-s, --source <path>",
      "Path to the svg icons source (./path/**.svg)"
    )
    .option(
      "-d, --destination [path]",
      "Destination path for the react components (./path/destination)"
    )
    .option(
      "-t, --template [path]",
      "Path to react component template (./path/template.js)"
    )
  ...
  console.log(chalk.green("Rock on! We exported your icons to " + destination));
}
...
function getTemplatePath(providedPath, typescript) {
  ...
  console.log("use default js template");
...
async function generateIcons({ source, template, destination, typescript }) {
  ...
  glob(source, (err, icons) => {
    ...
    icons.forEach(iconPath => {
      ...
      const svgString = $("svg")
        .toString()
        .replace(new RegExp('stroke="currentColor"', "g"), "stroke={stroke}")
